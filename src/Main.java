    ...
    public static void main(String[] args) {
        ...
        switch (answer) {
            case "ДА" -> {
                ...
                while (person.getLive() > 0) {
                    for (int i = 0; i < sizeBoard; i++) {
                        for (int j = 0; j < sizeBoard; j++) {
                            if (board[i][j].equals(person.getImage())) {
                                board[i][j] = "  ";
                            }
                        }
                    }
                    ...
                    if (person.moveCorrect(x, y)) {
                        ...
                        if (next.equals("  ")) {
                            ...
                            for (Monster monster : arrMonster) {
                                if (monster != null && monster.conflictPerson(x, y)) {
                                    if (monster.taskMonster(difficultGame)) {
                                        ...
                                        person.downLive();
                                        System.out.println("У вас осталось жизней: " + person.getLive());
                                    }
                                    monsterFound = true;
                                    break;
                                }
                            ...
                            if (!monsterFound) {
                                System.out.println("Ошибка: монстр не найден");
                            }
                        }
                    } else {
                ...
                if (person.getLive() <= 0) {
                    System.out.println("Игра окончена! У вас не осталось жизней.");
                }
            }
